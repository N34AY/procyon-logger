...
import { ILogger, ISendLogPayload, ISendLogResponse, IGetLogsOptions, IGetLogsResponse } from './types';
  ...
  async sendLog(payload: ISendLogPayload): Promise<ISendLogResponse> {
    try {
      ...
      if (!this.send && payload.send) {
        const message =
          'WARNING: Constructor takes "send: false" but you pass "send: true" in sendLog method. Log will not be sent.';
